%% Fine Regression Forest Training Script
...
for g = 1:32
    ...
    for t = 3:size(files,1)
        ...
        % Training at fine resolution, so image is upsampled by 2.
        out = imresize3(out,2);
        ijk_arr = [ijk_arr(1:3,:)*2; ijk_arr(4,:)];
        ijk_arr = round(ijk_arr);
        out = padarray(out,[50,50,50],0,'both');
        ijk_arr = [ijk_arr(1:3,:) + 50;ones(1,32)];

        if any(ijk_arr(1:3,g) < 121) == 1 
            continue
        end
        
        % Creates integral image from patch.
        patch = out(ijk_arr(1,g)-120:ijk_arr(1,g)+120,ijk_arr(2,g)-120:ijk_arr(2,g)+120,ijk_arr(3,g)-120:ijk_arr(3,g)+120);
        ...
        % the ground truth.
        coords1 = combvec(121-5:121+5,121-5:121+5,121-5:121+5);
        coords2 = combvec(121-10:2:121+10,121-10:2:121+10,121-10:2:121+10);
...
save(sprintf('mdl%d_fine.mat',g),'Mdl')
clear all
end